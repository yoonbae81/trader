...
public:
	...
	TEST_METHOD(PutTest) {
		...
		Assert::AreEqual(m.lastest_timestamp, sut.lastest_timestamp);
		...
		// when lastest_timestamp is same, subsequent request should update price and add volume
		m.price *= 2;
